...
static PyMethodDef methodsMethods[] = {
    ...
    {"calculate_surface_velocity", py_calculate_surface_velocity, METH_VARARGS,
        ...
        "Signature A — ensemble of EulerVectors:\n"
        "  ev_ensemble : list of EulerVector\n"
        "  lon, lat    : float — single point; returns tuple of 4 arrays\n"
        "  n_size      : int, optional for scalar lon/lat; number of\n"
        "                ensemble members to use from ev_ensemble.\n"
        "  lons, lats  : sequence of float — multiple points; returns list of tuples\n"
        "  Each tuple contains (east_vel, north_vel, total_vel, azimuth),\n"
        "  in cm/yr and degrees CW from North.\n\n"
        "Signature B — single EulerVector:\n"
        "  ev          : EulerVector (with or without covariance)\n"
        "  lon, lat    : float — single point; returns SurfaceVelocity\n"
        "  lons, lats  : sequence of float — multiple points; returns list\n"
        "  n_size      : int, optional (default 100000) — ensemble size when\n"
        "                covariance is present.\n"
        "  Returns Stat-based fields when covariance is present, float fields\n"
        "  otherwise."
    },
